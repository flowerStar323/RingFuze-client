...
// ** Table Data & Columns
import { data, columns } from '../../../@fake-db/apps/numbers'

// ** Invoice List Sidebar
import Sidebar from './Sidebar'

import { fetchNumbers } from '../../../redux/actions/numbers'
...
const NumbersTable = () => {
  ...
  const resNumbers = useSelector(state => state.numbers.data)
  ...
  useEffect(() => {
    if (resNumbers?.success) {
      setTableData(resNumbers.data)
    }
  }, [resNumbers])
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      tableData?.forEach((item => {
        if (item.username.includes(value) || item.email.includes(value) || item.number.includes(value)) {
  ...
  const CustomPagination = () => (
    <ReactPaginate
      ...
      pageCount={
        searchValue.length ? filteredData.length / 7 : tableData?.length / 7 || 1
      }
      breakLabel="..."
    ...
    <Fragment>
      <Card>
        ...
        <DataTable
          ...
          data={searchValue ? filteredData : tableData}
